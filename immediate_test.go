...
func TestNewImmediateHitCounter(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			c, err := counter.NewImmediateHitCounter(tt.duration, tt.resolution)
...
func TestImmediateRollingTicksWithVariableHits(t *testing.T) {
	for ti, tt := range []struct {
		...
		t.Run(testName, func(t *testing.T) {
			...
			c, err := counter.NewImmediateHitCounter(time.Second*time.Duration(tt.duration), time.Second)
			if err != nil {
